...
(defn post-sign-in [storage {:keys [request env]}]
  (js-await [data (cf/request->edn request)]
            (let [{:keys [user password]} data]
              (js-await [account (persistance/get-account storage user)]
                        (if (nil? account)
...
(defn post-sign-up [storage {:keys [request env]}]
  (js-await [data (cf/request->edn request)]
            (let [{:keys [email name password]} data]
              (js-await
               [account-res (persistance/create-account storage name email password)]
